...
for i in range(num_sections):
    ss = sec_start + i * 40
    sname = d[ss:ss+8].rstrip(b'\x00').decode('latin-1')
    ...
    if is_data: flags_parts.append('DATA')
    if is_exec: flags_parts.append('EXEC')
    if is_read: flags_parts.append('READ')
    if is_write: flags_parts.append('WRITE')
    
    print(f'  Sec[{i}] {sname}: RVA=0x{srva:08x} VSize=0x{svsize:x} RPtr=0x{srptr:x} RSize=0x{srsize:x} {"|".join(flags_parts)}', flush=True)
