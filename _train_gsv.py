...
用法：
  python _train_gsv.py preprocess   # 预处理（Hubert 特征 + 语义 token）
  python _train_gsv.py s1           # GPT 训练（文本->语义）
  python _train_gsv.py s2           # SoVITS 训练（语义->音频）
"""
import os, sys, json, subprocess, argparse

# 项目根目录：以本脚本位置为准，可移植到任意路径
BASE = os.path.dirname(os.path.abspath(__file__))
GSV = os.path.join(BASE, 'gpt-sovits')
PY = os.path.join(BASE, 'index-tts', '.venv', 'Scripts', 'python.exe')

VERSION = 'v2'
EXP_NAME = 'typhoea'
EXP_ROOT = os.path.join(GSV, 'logs')
OPT_DIR = os.path.join(EXP_ROOT, EXP_NAME)
INP_TEXT = os.path.join(BASE, 'typhoea_train_asr', 'typhoea.list')
INP_WAV = os.path.join(BASE, 'typhoea_train')
