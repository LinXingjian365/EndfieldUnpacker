...
用法（在项目根目录执行）：
  python _train_rvc.py preprocess    # 数据切分
  python _train_rvc.py f0            # RMVPE 音高提取
  python _train_rvc.py hubert        # HuBERT 特征提取
  python _train_rvc.py files         # 生成 filelist.txt + config.json
  python _train_rvc.py train         # 训练
  python _train_rvc.py all           # 依次执行以上全部
"""
import json
import os
import subprocess
...
RVC = os.path.join(BASE, "RVC")
PY = os.path.join(RVC, ".venv", "Scripts", "python.exe")

EXP = "typhoea"
INPUT = os.path.join(BASE, "typhoea_train")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法: python _train_rvc.py [preprocess|f0|hubert|files|train|all]")
        return 1
    arg = sys.argv[1]
