...
用法：
  python _infer_gsv.py
输出：
  gpt-sovits/output_typhoea_ft.wav / .mp3
"""
import os
import sys
...
REF_AUDIO = os.path.join(BASE, 'typhoea_train', 'typhoea_18645.wav')
...
def main():
    ...
    out_wav = os.path.join(GSV, 'output_typhoea_ft.wav')
    sf.write(out_wav, audio, sr)
    print('已保存 WAV:', out_wav, '采样率', sr, '时长 %.2fs' % (len(audio) / sr))

    # 转 MP3 便于手机播放
    out_mp3 = os.path.join(GSV, 'output_typhoea_ft.mp3')
    try:
        import subprocess
